...
def compute_projections(output_path, project_name, batch_id, features, path_images, predictions, labels, base_tsne=None, compute_base=False, save=True):
    if compute_base:
    ...
    df = pd.DataFrame(tsne_arr, columns =['names', 'x', 'y', 'pred', 'label'])
